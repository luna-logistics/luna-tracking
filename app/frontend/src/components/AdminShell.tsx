...
export function AdminShell() {
  ...
  const items: Array<{ to: string; label: string; icon: typeof LayoutDashboard; permission: AdminPermission | 'always' }> = ([
    ...
    { to: '/admin/magasins',               label: t('admin.sidebar_stores'),          icon: Store,           permission: 'products' },
  ...
  return (
    <div className="admin-shell min-h-screen bg-background flex">
      <aside className="hidden md:flex md:flex-col w-60 shrink-0 bg-luna-navy-deep text-white">
        ...
        {/* Nav items + admin actions live in the SAME scroll flow so the
            "back to site" and logout buttons sit right below the menu
            instead of being pinned to the bottom of the viewport (which
            forced admins to scroll on shorter screens). */}
