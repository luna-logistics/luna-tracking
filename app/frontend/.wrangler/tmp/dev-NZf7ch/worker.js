...
// .wrangler/tmp/bundle-qDO69O/strip-cf-connecting-ip-header.js
...
// worker.js
var StripElement = class {
  element(element) {
    element.remove();
  }
};
__name(StripElement, "StripElement");
var ReplaceTitle = class {
  constructor(text) {
    this.text = text;
  }
  element(element) {
...
async function serveNeutralShell(request, env) {
  ...
  const rewriter = new HTMLRewriter().on('link[rel="canonical"]', new StripElement()).on('link[rel="alternate"][hreflang]', new StripElement()).on('meta[name="description"]', new StripElement()).on('meta[property^="og:"]', new StripElement()).on('meta[name^="twitter:"]', new StripElement()).on('script[type="application/ld+json"]', new StripElement()).on("title", new ReplaceTitle("Luna Tracking Logistics"));
...
__name(serveNeutralShell, "serveNeutralShell");
var worker_default = {
...
// .wrangler/tmp/bundle-qDO69O/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-qDO69O/middleware-loader.entry.ts
