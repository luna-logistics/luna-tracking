...
// .wrangler/tmp/bundle-cqFnkn/strip-cf-connecting-ip-header.js
...
// worker.js
var stripHandler = { element(el) {
  el.remove();
} };
var neutralTitleHandler = {
  element(el) {
    el.setInnerContent("Luna Tracking Logistics");
  }
};
async function serveNeutralShell(request, env) {
  ...
  const rewriter = new HTMLRewriter().on('link[rel="canonical"]', stripHandler).on('link[rel="alternate"][hreflang]', stripHandler).on('meta[name="description"]', stripHandler).on('meta[property^="og:"]', stripHandler).on('meta[name^="twitter:"]', stripHandler).on('script[type="application/ld+json"]', stripHandler).on("title", neutralTitleHandler);
...
// .wrangler/tmp/bundle-cqFnkn/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-cqFnkn/middleware-loader.entry.ts
