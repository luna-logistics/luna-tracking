...
begin
  ...
  insert into public.invoice_lines (invoice_id, line_index, description, quantity, unit_price, vat_pct)
    values (inv, 0, 'Fret', 1, 100, 21);

  -- draft → issued by plain UPDATE must fail
  begin
    update public.invoices set status = 'issued' where id = inv;
    bad := bad || 'plain issue accepted';
  exception when others then
    if sqlerrm like 'invoice_issue_via_rpc%' then ok := ok || 'plain issue refused'; else bad := bad || ('issue: ' || sqlerrm); end if;
  end;
  ...
  begin
    ...
    bad := bad || 'notes edited after issue';
  exception when others then
    if sqlerrm like 'invoice_locked%' then ok := ok || 'header frozen'; else bad := bad || ('header: ' || sqlerrm); end if;
  end;
  begin
    update public.invoices set currency = 'USD' where id = inv;
    bad := bad || 'currency edited after issue';
  exception when others then
    if sqlerrm like 'invoice_locked%' then ok := ok || 'currency frozen'; else bad := bad || ('currency: ' || sqlerrm); end if;
  end;
  begin
    -- as the service role too: lines are frozen for everyone
    execute 'set local role service_role';
    insert into public.invoice_lines (invoice_id, line_index, description, quantity, unit_price, vat_pct)
      values (inv, 1, 'Extra', 1, 50, 21);
    bad := bad || 'line added after issue';
  exception when others then
    if sqlerrm like 'invoice_locked%' then ok := ok || 'lines frozen'; else bad := bad || ('lines: ' || sqlerrm); end if;
  end;
  execute 'set local role authenticated';

  begin
    update public.invoices set paid_via = 'stripe', status = 'paid' where id = inv;
    bad := bad || 'member wrote paid_via=stripe';
  exception when others then
    if sqlerrm like 'invoice_payment_fields_readonly%' then ok := ok || 'stripe fields guarded'; else bad := bad || ('stripe: ' || sqlerrm); end if;
  end;

  begin
    delete from public.invoices where id = inv;
    -- RLS limits deletes to drafts (0 rows) — also acceptable
    if exists (select 1 from public.invoices where id = inv) then ok := ok || 'issued not deletable';
    else bad := bad || 'issued invoice deleted'; end if;
  exception when others then
    if sqlerrm like 'invoice_locked%' then ok := ok || 'issued not deletable'; else bad := bad || ('delete: ' || sqlerrm); end if;
  end;

  update public.invoices set status = 'paid' where id = inv returning * into r;
  if r.status = 'paid' and r.paid_via = 'manual' and r.paid_on is not null then ok := ok || 'manual paid recorded';
  ...
  begin
    ...
    bad := bad || 'paid → cancelled accepted';
  exception when others then
    if sqlerrm like 'invoice_status_terminal%' then ok := ok || 'paid terminal'; else bad := bad || ('terminal: ' || sqlerrm); end if;
  end;
